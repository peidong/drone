...
float deltat = 0.011f;                             // integration interval for both filter schemes
...
void MPU_init()
{
  mraa_init();
	...
	mraa_i2c_write_byte_data(mpu, 0x06, ACCEL_CONFIG_2);
	mraa_i2c_write_byte_data(mpu, 0x06, CONFIG);
  ...
  //////////////////////// write offset of gyro
 // mraa_i2c_write_byte_data(mpu, gyro_offset_x/256, 19);
//  mraa_i2c_write_byte_data(mpu, gyro_offset_x, 20);
//  mraa_i2c_write_byte_data(mpu, gyro_offset_y/256, 21);
//  mraa_i2c_write_byte_data(mpu, gyro_offset_y, 22);
//  mraa_i2c_write_byte_data(mpu, gyro_offset_z/256, 23);
//  mraa_i2c_write_byte_data(mpu, gyro_offset_z%236, 24);
}
...
//  zeta = invSqrt(0.75) * PI * (1.0f / 180.0f);  // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
  ...
  float x=0,y=0,z=0;
    ...
    uint8_t Buf[14];
		...
		// Accelerometer
		int16_t arawx = (Buf[0] << 8 | Buf[1]);
		int16_t arawy = (Buf[2] << 8 | Buf[3]);
		int16_t arawz = Buf[4] << 8 | Buf[5];
		// Gyroscope
		int16_t grawx = -(-(Buf[8] << 8 | Buf[9]) + 29); 
		int16_t grawy = -(-(Buf[10] << 8 | Buf[11]));
		int16_t grawz = (Buf[12] << 8 | Buf[13]) + 0;
...
//		printf("%6d,%6d,%6d\n",grawx, grawy, grawz);
    ...
    float mx = (float)mrawx*mRes*magCalibration[0];  // get actual magnetometer value, this depends on scale being set
    float my = (float)mrawy*mRes*magCalibration[1];  
    float mz = (float)mrawz*mRes*magCalibration[2];
    /*
    if(sample<10){
          now = clock();
...
//    yaw   -= 13.8; // Declination at Danville, California is 13 degrees 48 minutes and 47 seconds on 2014-04-04
    ...
    result[sample][0] = yaw;
    result[sample][1] = pitch;
    result[sample][2] = roll;    
