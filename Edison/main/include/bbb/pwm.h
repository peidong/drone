...
int pwm_start(const char *key, float duty, float freq, int polarity)
{
    ...
    if ((run_fd = open(run_path, O_RDWR)) < 0){
        ...
        n_run_len = snprintf(c_run_buffer, sizeof(c_run_buffer), "%d", 0);
        write(run_fd, c_run_buffer, n_run_len);
    }
    ...
    n_run_len = snprintf(c_run_buffer, sizeof(c_run_buffer), "%d", 1);
    write(run_fd, c_run_buffer, n_run_len);
    close(run_fd);
    return 1;
}
