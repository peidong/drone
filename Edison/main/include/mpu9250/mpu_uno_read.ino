...
// char acc_str[12],gyro_str[12],mag_str[13];
char mpu_data[37];
...
float GyroMeasError = PI * (360.0f / 180.0f);   // gyroscope measurement error in rads/s (start at 40 deg/s)
...
{
  Wire.begin();
  TWBR = 10;  // 400 kbit/sec I2C speed
  Serial.begin(115200);
...
{  
  ...
  // if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {  // On interrupt, check if data ready interrupt
    ...
    // Serial.print(accelCount[1]);Serial.print("\t");
    // Serial.print(accelCount[2]);Serial.print("\t");
   ...
   readGyroData();  // Read the x/y/z adc values
    // getGres();
    // Serial.print(gyroCount[0]);Serial.print("\t");
    // Serial.print(gyroCount[1]);Serial.print("\t");
    // Serial.print(gyroCount[2]);Serial.println("\t");
   ...
   // delay(2);
    ...
    // mx = (float)magCount[0]*mRes*magCalibration[0] - 533 - 60;  // get actual magnetometer value, this depends on scale being set
    // my = (float)magCount[1]*mRes*magCalibration[1] - 215 + 78;  
    // mz = (float)magCount[2]*mRes*magCalibration[2] + 254 - 53;   
...
/**********************************************************
  ...
  // MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
	...
	delt_t = millis() - count;
	if (delt_t > count_t) { // update LCD once per half-second independent of read rate
...
{  
 ...
 // With the MPU9250, it is possible to get gyro sample rates of 32 kHz (!), 8 kHz, or 1 kHz
  writeByte(MPU9250_ADDRESS, CONFIG, 0x00);  //0.97ms delay time
...
//  writeRegister(ACCEL_CONFIG, c & ~0xE0); // Clear self-test bits [7:5] 
 ...
 // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
  ...
  writeByte(MPU9250_ADDRESS, ACCEL_CONFIG2, c | 0x08); // Set accelerometer rate to 4 kHz and bandwidth to 1.13kHz
