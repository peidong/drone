...
#pragma mark - Peidong
+(BOOL)writeFileAtPath:(NSString *)path content:(NSString *)content append:(BOOL)isAppend newline:(BOOL)isNewLine{
    if ([FCFileManager isFileContentNilAtPath:path]) {
        return [FCFileManager writeFileAtPath:path content:content error:nil];
    }
    if (isAppend) {
        NSString *originalFileContent = [FCFileManager readFileAtPath:path];
        NSString *addingFileString = content;
        if (isNewLine) {
            NSString *temp=[originalFileContent stringByAppendingString:@"\n"];
            NSString *newFileContent=[temp stringByAppendingString:addingFileString];
            return [FCFileManager writeFileAtPath:path content:newFileContent error:nil];
        }else{
            NSString *newFileContent=[originalFileContent stringByAppendingString:addingFileString];
            return [FCFileManager writeFileAtPath:path content:newFileContent error:nil];
        }
...
+(BOOL)writeFileAtDocumentPathWithFileName:(NSString *)fileName Content:(NSString *)content append:(BOOL)isAppend newline:(BOOL)isNewLine{
    NSString *DocumentsDirectory = [FCFileManager pathForDocumentsDirectory];
    NSString *tempPath = [DocumentsDirectory stringByAppendingString:@"/"];
    NSString *path = [tempPath stringByAppendingString:fileName];
    return [self writeFileAtPath:path content:content append:isAppend newline:isNewLine];
}

+(BOOL)isFileContentNilAtPath:(NSString *)path{
    if (![self isFileItemAtPath:path]) {
        return true;
    }else{
    NSString *fileContent = [self readFileAtPath:path];
...
+(BOOL)isFileContentNilAtDocumentPathWithFileName:(NSString *)fileName{
    NSString *DocumentsDirectory = [FCFileManager pathForDocumentsDirectory];
    NSString *tempPath = [DocumentsDirectory stringByAppendingString:@"/"];
    NSString *path = [tempPath stringByAppendingString:fileName];
    return [self isFileContentNilAtPath:path];
}

+(NSString *)readFileAtDocumentPathFileName:(NSString *)fileName{
    NSString *DocumentsDirectory = [FCFileManager pathForDocumentsDirectory];
    NSString *tempPath = [DocumentsDirectory stringByAppendingString:@"/"];
    NSString *path = [tempPath stringByAppendingString:fileName];
    return [self readFileAtPath:path];
}
